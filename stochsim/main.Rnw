...
\title{Simulation of stochastic dynamic models}
...
\begin{frame}[allowframebreaks]{A basic compartment model: The SIR model}

  \begin{itemize}
  \item
    We develop deterministic and stochastic representations of a susceptible-infected-recovered (SIR) system, a simple example of a broad class of models for disease transmission dynamics.
  \end{itemize}
  ...
  \begin{tabular}{c @{\ :\ } l c @{\ :\ } l}
    $S$ & susceptible hosts & $I$ & infected and infectious hosts \\
    $R$ & recovered and/or removed hosts & $C$ & reported cases
  \end{tabular}
  ...
  \item We suppose that each arrow has an associated rate, so here there is a rate $\mu^{}_{SI}(t)$ at which individuals in $S$ transition to $I$, and $\mu^{}_{IR}$ at which individuals in $I$ transition to $R$.
  \item To account for demography (births/deaths/migration) we allow the possibility of a source and sink compartment, which is not usually represented on the flow diagram. We write $\mu^{}_{BS}$ for a rate of births into $S$, and denote mortality rates by $\mu^{}_{SD}$, $\mu^{}_{ID}$, $\mu^{}_{RD}$.
  \item The rates may be either constant or varying. In particular, for a simple SIR model, the recovery rate $\mu^{}_{IR}$ is a constant but the infection rate has the time-varying form
    $$\mu^{}_{SI}(t)=\beta \, I(t),$$
    with $\beta$ being the \myemph{transmission rate}.
    For the simplest SIR model, we ignore demography, setting
    $$ \mu^{}_{BS}=\mu^{}_{SD}=\mu^{}_{ID}=\mu^{}_{RD}=0.$$
  \end{itemize}
\end{frame}
...
\begin{frame}{Ordinary differential equation interpretation}
  ...
  \begin{equation*}
    \begin{aligned}
      \frac{\dd{N_{SI}}}{\dd{t}} &= \mu^{}_{SI}(t)\,S(t)\\
      \frac{\dd{N_{IR}}}{\dd{t}} &= \mu^{}_{IR}\,I(t)
    \end{aligned}
  \end{equation*}
\end{frame}
...
\begin{frame}{Continuous-time Markov chain interpretation}
  ...
  \item The Markov property lets us specify a model by the transition probabilities on small intervals (together with the initial conditions). For the SIR model, we have
    ...
    \begin{equation*}
      \begin{aligned}
        \pr\big[N_{SI}(t+\delta)&\equals N_{SI}(t)+1\big] &\equals& \phantom{1-}\mu^{}_{SI}(t)\,S(t)\,\delta + o(\delta)
        \\
        \pr\big[N_{SI}(t+\delta)&\equals N_{SI}(t)\big] &\equals& 1-\mu^{}_{SI}(t)\,S(t)\,\delta + o(\delta)
        \\
        \pr\big[N_{IR}(t+\delta)&\equals N_{IR}(t)+1\big] &\equals& \phantom{1-}\mu^{}_{IR}\,I(t)\,\delta + o(\delta)
        \\
        \pr\big[N_{IR}(t+\delta)&\equals N_{IR}(t)\big] &\equals& 1-\mu^{}_{IR}(t)\,I(t)\,\delta + o(\delta)
      \end{aligned}
    \end{equation*}
...
\begin{frame}[allowframebreaks=0.8]{Euler's method for a discrete SIR model}
  ...
  \item Recall the simple continuous-time Markov chain interpretation of the SIR model without demography:
    \begin{equation*}
      \begin{aligned}
        \pr\big[N_{SI}(t+\delta)&\equals N_{SI}(t)+1\big] &\equals& \mu^{}_{SI}(t) \, S(t) \delta + o(\delta),
        \\
        \pr\big[N_{IR}(t+\delta)&\equals N_{IR}(t)+1\big] &\equals& \mu^{}_{IR} \, I(t) \delta + o(\delta).
      \end{aligned}
    \end{equation*}
...
\begin{frame}{Three different stochastic Euler solutions}
  ...
  \item A Poisson approximation.
    $$\tilde N_{SI}(t+\delta)= \tilde N_{SI}(t) + \mathrm{Poisson}\big[\mu^{}_{SI}\big(\tilde I(t)\big) \, \tilde S(t) \,\delta\big],$$
    where $\mathrm{Poisson}(\mu)$ is a Poisson random variable with mean $\mu$ and
    $$\mu^{}_{SI}\big(\tilde I(t)\big) = \beta\, \tilde I(t).$$

  \item A binomial approximation,
    $$\tilde N_{SI}(t+\delta) = \tilde N_{SI}(t) + \mathrm{Binomial}\big[\tilde S(t),\mu^{}_{SI}\big(\tilde I(t)\big) \, \delta\big],$$
    where $\mathrm{Binomial}(n,p)$ is a binomial random variable with mean $np$ and variance $np(1-p)$. Here, $p=\mu^{}_{SI}\big(\tilde I(t)\big) \, \delta$.

  \item A binomial approximation with exponential transition probabilities.
    $$ \tilde N_{SI}(t+\delta)= \tilde N_{SI}(t) + \mathrm{Binomial}\big[\tilde S(t),1-\exp\big\{-\mu^{}_{SI}\big(\tilde I(t)\big) \delta\big\}\big].$$
...
\begin{frame}[allowframebreaks=0.9]{Compartment models as stochastic differential equations}
  ...
  \item A natural way to add stochastic variation to an ODE $dx/dt=h(x)$ is
    \begin{equation*}
      \frac{\dd{X}}{\dd{t}} = h(X)+\sigma\,\frac{\dd{B}}{\dd{t}}
    \end{equation*}
...
\begin{frame}[fragile,allowframebreaks]{The Consett measles outbreak}
  ...
  <<meas-data2,echo=FALSE,out.width="70%">>=
  library(tidyverse)
  meas |>
...
\begin{frame}[fragile]{A simple POMP model for measles}
  ...
  \item The rate at which individuals move from S to I is the \myemph{force of infection}, $\mu^{}_{SI}=\beta\,I/N$, while that at which individuals move into the R class is $\mu^{}_{IR}$.
  \end{itemize}
\end{frame}
...
\begin{frame}[fragile,allowframebreaks]{Implementing the SIR model in \pkg{pomp}}
  ...
  \item  We follow method 3 above, modeling the number, $\Delta{N_{SI}}$, moving from S to I over interval $\Delta{t}$ as
    ...
    $$\Delta{N_{IR}} \sim \dist{Binomial}{I,1-e^{-\mu^{}_{IR}\,\Delta{t}}}.$$
...
\begin{frame}[fragile,allowframebreaks]{Guessing plausible parameter values}
  ...
  \item If the infectious period is roughly 2 weeks, then $1/\mu^{}_{IR} \approx 2~\text{wk}$ and $\beta = \mu^{}_{IR}\,\Rzero \approx 7.5~\text{wk}^{-1}$.
...
\begin{frame}{\myexercise. The SEIR model}
  ...
  \input{../graphics/seir_diagram}

  <<seir-diagram,child="../graphics/seir_diagram.Rnw",echo=FALSE,purl=FALSE,fig.height=3/4,fig.width=15/2,eval=FALSE>>=
  @
