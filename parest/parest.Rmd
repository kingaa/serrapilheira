...
For your convenience, the **R** codes for this document are `r xfun::embed_file("parest.R",text="provided in a script")` which you can download, edit, and run.
...
$$\widehat{R_{0}}\;=\;\frac{\mathrm{slope}}{\gamma}+1\;\approx\;{`r signif(slope,2)`}\times{1}+1\;\approx\;{`r signif(slope*1+1,2)`}.$$
...
slope.se
```
So we reckon we've got an error of $\pm{`r signif(1*slope.se,1)`}$ in our estimate of $R_0$, i.e., we feel reasonably confident that ${`r signif((slope-2*slope.se)*1+1,3)`}<R_{0}<{`r signif((slope+2*slope.se)*1+1,3)`}$.
...
niamey |>
  group_by(community) |>
  summarize(
    measles=n()*mean(measles,na.rm=TRUE)
  ) |>
  ungroup() -> tots
...
fitfn <- function (dat) {
  ...
  slope <- coef(summary(fit))[2,1]
  slope.se <- coef(summary(fit))[2,2]
  data.frame(R0.hat=slope*1+1,R0.se=slope.se*1)
}
...
niamey |>
  filter(measles>0,!is.na(measles)) |>
  group_by(community) |>
  slice(1:8) |>
  reframe(fitfn(pick(biweek,measles))) |>
  ungroup() |>
  group_by(community) |>
  mutate(f=finalsize(R0.hat)) |>
  ungroup() |>
  left_join(
    tots,
    by="community"
...
expand_grid(
  ...
  group_by(b,p) |>
  mutate(loglik=f6(b,p)) |>
  ungroup() |>
  filter(is.finite(loglik)) |>
  ggplot(aes(x=b,y=p,z=loglik,fill=loglik))+
  geom_tile()+
  geom_contour(binwidth=2000)
```
