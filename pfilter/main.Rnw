...
\title{Likelihood-based inference for POMP models}
...
\begin{frame}[allowframebreaks]{Overview}
  ...
  <<flow-diagram,child="../graphics/flow_diagram.Rnw",fig.dim=c(4.5,4),fig.align="center",echo=FALSE,results="hide",purl=FALSE,out.width="45%">>=
  @
...
\begin{frame}[allowframebreaks]{Sequential Monte Carlo: The particle filter}
  ...
  \begin{itemize}
  \item References on the particle filter include \citet{Kitagawa1987}, \citet{Arulampalam2002}, \citet{Doucet2001}, \citet{King2016}.
...
\begin{frame}{A block diagram representation of a particle filter}
...
\begin{frame}[fragile,allowframebreaks=0.8]
  ...
  \item using the parallel for loop provided by the \pkg{foreach} package; and
  ...
  The first three lines above load the \pkg{foreach}, \pkg{iterators}, and \pkg{doFuture} packages, the latter being a ``backend'' for the \pkg{foreach} package.
  The next line tells \pkg{foreach} that we will use the \pkg{doFuture} backend.
  The last line says that we plan to parallelize using multiple \Rlanguage sessions on our single machine.
  By default, \Rlanguage will attempt to determine how many cores are available and will run an appropriate number of concurrent \Rlanguage processes.

\end{frame}

\begin{frame}[fragile,allowframebreaks]{Particle filtering in \pkg{pomp}}
  ...
  Here, we repeat the construction of the SIR model \link{https://kingaa.github.io/serrapilheira/stochsim/}{we looked at earlier}, using the parameters chosen by looking at simulations.
  \Rlanguage code to build the model is available \link{./model.R}{for download}.
  We can execute this code by sourcing the relevant file:
  <<model-construct,purl=TRUE>>=
  source("https://kingaa.github.io/serrapilheira/pfilter/model.R")
  @

  In \pkg{pomp}, the basic particle filter is implemented in the command \code{pfilter}.
  ...
  Note that we set the parallel RNG seed in the \code{foreach} call.
...
\begin{frame}{Profile likelihood estimation}
  ...
  We will explain this method below and put it into practice in the \link{https://kingaa.github.io/serrapilheira/mif/}{next lesson}.
\end{frame}
...
\begin{frame}{\myexercise. Cost of a particle-filter calculation}
  ...
  \link{./expense.html}{Worked solution to the Exercise}
\end{frame}
...
\begin{frame}{\myexercise. log-likelihood estimation}
  ...
  \item
    Use \pkg{doFuture} to take advantage of multiple cores on your computer to improve your estimate.
  \end{itemize}
...
\begin{frame}[allowframebreaks]{Wilks' theorem and profile likelihood}
  ...
  \item
    This is an example of \link{https://www.stat.nus.edu.sg/~wloh/lecture17.pdf}{a general duality between confidence intervals and hypothesis tests}.
  \end{itemize}
\end{frame}
