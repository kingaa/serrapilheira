...
Questions about the mechanistic operation of biological systems are naturally formulated as stochastic processes, but confronting such models with data can be challenging.
In this talk, I describe the essence of the difficulty, highlighting both the technical issues and the importance of the "plug-and-play property".
I then illustrate some effective approaches to efficient inference based on such models.
I conclude by sketching promising new developments and describing some open problems.
-->
...
$m$ is an activity of $M$, the set of mathematicians.
$M\subset B$, where $B$ is the set of biological organisms.
Therefore, $b$ is at least as interesting as $m$.
Moreover, there is at least one element of $b$ that has not yet been mathematically studied.
Q.E.D.
...
- Predictive power
	...
	- What predictions to parameter estimates make about the results of different kinds of studies?
- Fertility
...
```{r dacca-profile1,fig.height=3,echo=FALSE}
...
```{r dacca-profile2,fig.height=3,echo=FALSE}
read.csv("data/mle.csv") |>
  filter(dataset=="Dacca") |>
  mutate(
    fatality=clin*deltaI/(delta+deltaI+eps),
